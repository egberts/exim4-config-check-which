...
if [ ! -f /etc/os-release ]; then
  echo "Probably not a Debian OS distro; aborted."
  exit 1
fi
source /etc/os-release
if [ "$ID" == 'debian' ] || [ "$ID" == 'devuan' ]; then
  echo "Detected this host as a Debian OS distro."
else
  echo "Not a Debain OS distro; aborted."
  exit 1
fi
...
elif [ -f "$EXIM_CONF_FILESPEC" ]; then
...
if [ -f "$EXIM_CONF_FILESPEC" ]; then
  ...
  echo "Exim4 daemon reads ONLY from '$EXIM_CONF_FILESPEC'"
...
echo -n "Performing simple syntax-check: "
